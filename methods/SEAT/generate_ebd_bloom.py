""" Generate BLOOM embeddings, execute SEAT method, and save results """
import pickle
import datetime
import time
import math
                                        ...
                                        if '-' in sent.split()[idx]:  # here: special case of subword tokenization
                                              ...
                                              if subword_ids[idx_sent].count(idx) == 1:  # case: no subword tokenization
                                                    ...
                                                    if encoding == 'word-average':
                                                          ...
                                                          idx_new = len(subword_ids[idx_sent]) - subword_ids[idx_sent][::-1].index(idx) - 1
...
selected_tests = ['c1_name', 'c3_name', 'c6_name', 'c9_term', 'dis_term', 'occ_name', 'i1_name', 'i2_name']
results = []

for test in all_tests:
      embeds = bloom(sent_dict, test)
...
results_path = time.strftime("%Y%m%d-%H%M%S") + '_SEAT_bloom_reddit.csv'
