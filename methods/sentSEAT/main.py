...
from methods.sentSEAT import data, weat
from methods.sentSEAT.encoders import bert, elmo, gpt2
...
def main(models, tests, encodings, contexts, evaluations, parametric):
    """ Main function of sentSEAT method"""
                ...
                if measure == 'cosine':
                    for context in contexts:
                        ...
                        for encoding in encodings:
                            if model == 'elmo':
                                model = elmo.load_model()
                                encs_targ1 = elmo.encode(model, sents_targ1, stimuli_targ1, encoding)
                                encs_targ2 = elmo.encode(model, sents_targ2, stimuli_targ2, encoding)
                                encs_attr1 = elmo.encode(model, sents_attr1, stimuli_attr1, encoding)
                                encs_attr2 = elmo.encode(model, sents_attr2, stimuli_attr2, encoding)
                            elif model == 'bert':
                                model, tokenizer, subword_tokenizer = bert.load_model()
                                encs_targ1 = bert.encode(model, tokenizer, subword_tokenizer, sents_targ1, stimuli_targ1, encoding)
                                encs_targ2 = bert.encode(model, tokenizer, subword_tokenizer, sents_targ2, stimuli_targ2, encoding)
                                encs_attr1 = bert.encode(model, tokenizer, subword_tokenizer, sents_attr1, stimuli_attr1, encoding)
                                encs_attr2 = bert.encode(model, tokenizer, subword_tokenizer, sents_attr2, stimuli_attr2, encoding)
                            elif model == 'gpt2':
                                model, tokenizer, subword_tokenizer = gpt2.load_model()
                                encs_targ1 = gpt2.encode(model, tokenizer, subword_tokenizer, sents_targ1, stimuli_targ1, encoding)
                                encs_targ2 = gpt2.encode(model, tokenizer, subword_tokenizer, sents_targ2, stimuli_targ2, encoding)
                                encs_attr1 = gpt2.encode(model, tokenizer, subword_tokenizer, sents_attr1, stimuli_attr1, encoding)
                                encs_attr2 = gpt2.encode(model, tokenizer, subword_tokenizer, sents_attr2, stimuli_attr2, encoding)
                            else:
